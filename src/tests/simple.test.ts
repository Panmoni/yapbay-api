...
describe.skip('Simple Multi-Network Tests (Celo - DISABLED)', () => {
  ...
  describe('Network Configuration', () => {
    it('should have active networks configured', async () => {
    ...
    it('should get network by name', async () => {
      ...
      expect(alfajores?.chainId).to.equal(44_787);
      expect(mainnet?.chainId).to.equal(42_220);
    });
  });

  describe('Database Network Isolation', () => {
    beforeEach(async () => {
  ...
  describe('Cross-Network Data Prevention', () => {
    beforeEach(async () => {
      await client.query('BEGIN');
    });
    ...
    it('should not find offers from other networks', async () => {
      const alfajores = await NetworkService.getNetworkByName(NetworkType.CELO_ALFAJORES);
      const mainnet = await NetworkService.getNetworkByName(NetworkType.CELO_MAINNET);
