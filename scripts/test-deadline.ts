...
const _UNCANCELABLE_STATES = ['FIAT_PAID', 'RELEASED', 'DISPUTED', 'RESOLVED'];
...
(async () => {
  ...
  try {
    ...
    if (
      cancelableResult.rows[0].overall_status === 'CANCELLED' &&
    ...
    if (
      uncancelableResult.rows[0].overall_status === 'IN_PROGRESS' &&
