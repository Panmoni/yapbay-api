...
async function main() {
  ...
  try {
    ...
    results.push(
      await runTest('Network Configuration', async () => {
        const networks = await NetworkService.getActiveNetworks();
        if (networks.length < 2) {
          throw new Error('Expected at least 2 active networks');
        }
        ...
        if (!alfajores) {
          throw new Error('Alfajores network not found');
        }
        if (!mainnet) {
          throw new Error('Mainnet network not found');
        }
        if (alfajores.chainId !== 44_787) {
          throw new Error('Alfajores wrong chain ID');
        }
        if (mainnet.chainId !== 42_220) {
          throw new Error('Mainnet wrong chain ID');
        }
      }),
    );
    ...
    results.push(
      await runTest('Offers Data Isolation', async () => {
        ...
        try {
          ...
          // Verify isolation
          const alfajoresOffers = await client.query('SELECT * FROM offers WHERE network_id = $1', [
            alfajores!.id,
          ]);
          ...
          if (alfajoresOffers.rows.length !== 1) {
            throw new Error('Alfajores should have 1 offer');
          }
          if (mainnetOffers.rows.length !== 1) {
            throw new Error('Mainnet should have 1 offer');
          }
          if (alfajoresOffers.rows[0].offer_type !== 'BUY') {
            throw new Error('Alfajores offer type wrong');
          }
          if (mainnetOffers.rows[0].offer_type !== 'SELL') {
            throw new Error('Mainnet offer type wrong');
          }
        } finally {
    ...
    results.push(
      await runTest('Trades Data Isolation', async () => {
        await client.query('BEGIN');
        ...
        try {
          ...
          // Verify isolation
          const alfajoresTrades = await client.query('SELECT * FROM trades WHERE network_id = $1', [
            alfajores!.id,
          ]);
          ...
          if (alfajoresTrades.rows.length !== 1) {
            throw new Error('Alfajores should have 1 trade');
          }
          if (mainnetTrades.rows.length !== 1) {
            throw new Error('Mainnet should have 1 trade');
          }
          if (alfajoresTrades.rows[0].from_fiat_currency !== 'USD') {
            throw new Error('Alfajores currency wrong');
          }
          if (mainnetTrades.rows[0].from_fiat_currency !== 'EUR') {
            throw new Error('Mainnet currency wrong');
          }
        } finally {
          await client.query('ROLLBACK');
        }
    ...
    results.push(
      await runTest('Cross-Network Prevention', async () => {
        await client.query('BEGIN');
    ...
    results.push(
      await runTest('Deadline Service Network Isolation', async () => {
        await client.query('BEGIN');
        ...
        try {
          ...
          const accountResult = await client.query(
            'INSERT INTO accounts (wallet_address, username, email) VALUES ($1, $2, $3) RETURNING id',
            [
              '0xTest4234567890123456789012345678901234567890',
              'deadlineuser',
