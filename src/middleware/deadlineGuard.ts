 ...
 */
export function deadlineGuard(deadlineField: string, errorMsg: string) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    if (
      UNCANCELABLE_STATES.includes(trade.leg1_state) ||
      UNCANCELABLE_STATES.includes(trade.leg2_state)
    ) {
      res.status(400).json({
