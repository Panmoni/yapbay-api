...
export class EscrowMonitoringService {
  ...
  private arbitratorKeypair?: Keypair;
  private networkId!: number;
  private networkFamily!: string;
  ...
  static async createForNetwork(networkId: number): Promise<EscrowMonitoringService> {
    ...
    service.networkFamily = network.networkFamily;

    if (network.networkFamily === 'evm') {
      // Handle EVM networks (Celo)
      const arbitratorPrivateKey = process.env.CELO_PRIVATE_KEY;
      ...
      try {
        ...
        console.log(`✅ Created Solana escrow monitoring service for ${network.name}`);
        console.log(`🔑 Arbitrator keypair: ${service.arbitratorKeypair.publicKey.toBase58()}`);

        // TODO: Implement Solana-specific escrow monitoring
        console.warn(`⚠️  Solana escrow monitoring implementation pending for ${network.name}`);
        return service;
      } catch (error) {
        throw new Error(
  ...
  async monitorAndCancelExpiredEscrows(): Promise<void> {
    const network = await NetworkService.getNetworkById(this.networkId);
    const networkName = network ? network.name : `Network ${this.networkId}`;
    ...
    try {
      // Handle different network families
      if (this.networkFamily === 'evm') {
        await this.monitorEvmEscrows();
      } else if (this.networkFamily === 'solana') {
        await this.monitorSolanaEscrows();
      } else {
  ...
  /**
   * Monitor EVM escrows (existing implementation)
   */
  private async monitorEvmEscrows(): Promise<void> {
  ...
  private async monitorSolanaEscrows(): Promise<void> {
    ...
    // This will require:
    // 1. Querying Solana program for active escrows
    // 2. Checking expiration conditions
    // 3. Executing auto-cancellation transactions
  ...
  private async checkEligibilityWithDelay(escrowId: number, delayHours: number): Promise<boolean> {
    try {
      ...
      if (escrowDetails.state === 0 && escrowDetails.depositDeadline > 0) {
        // CREATED state
      ...
      if (escrowDetails.state === 1 && escrowDetails.fiatDeadline > 0 && !escrowDetails.fiatPaid) {
        // FUNDED state
  ...
  private async getEscrowDetailsFromContract(escrowId: number): Promise<EscrowDetails> {
    try {
      if (!this.contract) {
        throw new Error('Contract not initialized - only EVM networks supported');
      }
  ...
  async validateAllEscrowBalances(): Promise<void> {
    ...
    if (!this.contract) {
      console.log('[EscrowMonitor] Balance validation only supported for EVM networks');
      return;
    }
...
export async function monitorExpiredEscrows(): Promise<void> {
  ...
  try {
    // Get only active networks and monitor each one
    const networks = await NetworkService.getActiveNetworks();

    if (networks.length === 0) {
      console.log('[EscrowMonitor] No active networks found for monitoring');
      return;
    }

    console.log(`[EscrowMonitor] Monitoring ${networks.length} active networks`);
